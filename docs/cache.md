...
- **Redução drástica de latência:** Requisições em cache respondem em menos de 5ms (muitas vezes < 1ms), comparado aos 100ms–800ms de chamadas de rede externas.
...
- **Algoritmo LRU (*Least Recently Used*):** Mantém as entradas mais recentemente acessadas e descarta automaticamente as menos utilizadas assim que a capacidade máxima (`max: 1000` por padrão) é atingida, prevenindo qualquer risco de vazamento de memória (*memory leak*).
...
- **Sincronia com Timers:** Configurado com `perf: { now: () => Date.now() }`, garantindo compatibilidade imediata com testes automatizados e fake timers (`vi.useFakeTimers()`).
